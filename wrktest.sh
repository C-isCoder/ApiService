...
1. 执行命令： ./wrktest.sh diff apiservice1_wrk.dat http_wrk.dat
 
> Note: 需要确保系统安装了 wrk 和 gnuplot 工具
EOF

t1="apiservice" # 对比图中红色线条名称
t2="http" # 对比图中粉色线条名称
jobname="apiservice" # 本次测试名称
...
if [ "$1" != "" ];then
	...
	url="http://127.0.0.1:6663/sd/health"
fi
